 ...
 namespace facebook {
  ...
  Cache<Allocator>::Cache(const CacheConfig& config,
    ...
    if (!isRamOnly()) {
      ...
      if (config_.navyZNSKVCacheSizePct > 0) {
          XLOG(INFO) << "looking at ZNSKVCache config" << std::endl;
          nvmConfig.navyConfig.setZNSKVCache(config_.navyZNSKVCacheSizePct,
                                           config_.navySmallItemMaxSize,
                                           config_.navyZNSKVCacheBucketSize,
                                           config_.navyZNSKVCacheBucketRatio,
                                           config_.navyZNSKVCacheBloomFilterFp,
                                           config_.navyZNSKVCacheElementCountPerPBF,
                                           config_.navyZNSKVCacheEnableWriteBack,
                                           config_.navyZNSKVCacheStartStatZone,
                                           config_.navyZNSKVCacheStep,
                                           config_.navyZNSKVCacheEnableDoubleHash,
                                           config_.navyZNSKVCacheEnableDoubleBuffer,
                                           config_.navyZNSKVCacheEnableKickOff,
                                           config_.navyZNSKVCacheMaxNumKicks);
      }
