  ...
  if (!isRamOnly()) {
    ...
    if (config_.navyKangarooSizePct > 0) {
        ...
        nvmConfig.navyConfig.setKangaroo(config_.navyKangarooSizePct,
                                         ...
                                         config_.navyKangarooFlushingThreshold,
                                         config_.navyKangarooGcUpperThreshold,
                                         config_.navyKangarooGcLowerThreshold,
                                         config_.navyKangarooSetOverprovisioning
                                        );
    }
...
Stats Cache<Allocator>::getStats() const {
  ...
  if (!isRamOnly() && !navyStats.empty()) {
    ...
    ret.logToSetPysicalWrite_ = lookup("logToSetPysicalWrite_");
    ret.coldSetGCPysicalWrite_ = lookup("coldSetGCPysicalWrite_");
    ret.logToSetLogicalWrite_ = lookup("logToSetLogicalWrite_");
    ret.hotSetGCPysicalWrite_ = lookup("hotSetGCPysicalWrite_");
    ret.coldSetGCLogicalWrite_ = lookup("coldSetGCLogicalWrite_");
    ret.hotSetGCLogicalWrite_ = lookup("hotSetGCLogicalWrite_");
    ret.socLogicalWritten = lookup("navy_bh_logical_written");
    ret.fwLogPyhsicalWritten = lookup("navy_bh_log_physical_written");
    ret.fwSetPyhsicalWritten = lookup("navy_bh_set_physical_written");
    double bhLogicalBytes = lookup("navy_bh_logical_written");//soc
    double bcLogicalBytes = lookup("navy_bc_logical_written");//loc
