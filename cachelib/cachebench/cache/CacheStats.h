...
struct Stats {
  ...
  uint64_t socLogicalWritten{0};
  uint64_t fwLogPyhsicalWritten{0};
  uint64_t fwSetPyhsicalWritten{0};
  uint64_t logToSetPysicalWrite_{0};
  uint64_t coldSetGCPysicalWrite_{0};
  uint64_t hotSetGCPysicalWrite_{0};
  uint64_t logToSetLogicalWrite_{0};
  uint64_t coldSetGCLogicalWrite_{0};
  uint64_t hotSetGCLogicalWrite_{0};
  ...
  void render(std::ostream& out) const {
    ...
    if (numNvmGets > 0 || numNvmDeletes > 0 || numNvmPuts > 0) {
      ...
      out << folly::sformat("NVM dev write amplification   : {:6.2f}\n",
                            devWriteAmp);
      out << folly::sformat("socLogicalWritten   : {:6.2f} GB\n",
        socLogicalWritten / GB);
      out << folly::sformat("fwLogPyhsicalWritten   : {:6.2f} GB\n",
        fwLogPyhsicalWritten / GB);
      out << folly::sformat("fwSetPyhsicalWritten   : {:6.2f} GB\n",
        fwSetPyhsicalWritten / GB);
       out << folly::sformat("logToSetPysicalWrite_   : {:6.2f} GB\n",
        logToSetPysicalWrite_ / GB);
      out << folly::sformat("logToSetLogicalWrite_   : {:6.2f} GB\n",
        logToSetLogicalWrite_ / GB);
      out << folly::sformat("coldSetGCPysicalWrite_   : {:6.2f} GB\n",
        coldSetGCPysicalWrite_ / GB);
      out << folly::sformat("coldSetGCLogicalWrite_   : {:6.2f} GB\n",
        coldSetGCLogicalWrite_ / GB);
      out << folly::sformat("hotSetGCPysicalWrite_   : {:6.2f} GB\n",
        hotSetGCPysicalWrite_ / GB);
      out << folly::sformat("hotSetGCLogicalWrite_   : {:6.2f} GB\n",
        hotSetGCLogicalWrite_ / GB);
    }
