...
struct CacheConfig : public JSONConfig {
  ...
  // Determines how much of the device is given to ZNSKVCache engine in Navy
  uint64_t navyZNSKVCacheSizePct = 0;

  // ZNSKVCache Bucket Size determines how big each bucket is and what
  // is the phsyical write granularity onto the device.
  uint64_t navyZNSKVCacheBucketSize = 4096;

  // ZNSKVCache PBFGList buckets' ratio of total buckets.
  uint64_t navyZNSKVCacheBucketRatio = 1;

  // BloomFilter fp
  double navyZNSKVCacheBloomFilterFp = 0.001;

  // elements per BloomFilter (page level)
  uint64_t navyZNSKVCacheElementCountPerPBF = 40;

  // if enable writeback mode
  bool navyZNSKVCacheEnableWriteBack{false};

  // evict start zone
  uint64_t navyZNSKVCacheStartStatZone = 20;

  // evict step
  uint64_t navyZNSKVCacheStep = 5;

  // if enable double Hash
  bool navyZNSKVCacheEnableDoubleHash{false};

  // if enable double dataRegionBuffer
  bool navyZNSKVCacheEnableDoubleBuffer{false};

  // if kick objects when there is no enough space in dataRegion bucket
  bool navyZNSKVCacheEnableKickOff{false};

  // max number of kicking objects (Cuckoo Hash)
  uint64_t navyZNSKVCacheMaxNumKicks = 40;
