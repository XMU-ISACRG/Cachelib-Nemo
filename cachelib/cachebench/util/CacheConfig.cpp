...
CacheConfig::CacheConfig(const folly::dynamic& configJson) {
  ...
  JSONSetVal(configJson, navyKangarooLogWriteGranularity);

  JSONSetVal(configJson, navyKangarooFlushingThreshold);
  JSONSetVal(configJson, navyKangarooGcUpperThreshold);
  JSONSetVal(configJson, navyKangarooGcLowerThreshold);
  JSONSetVal(configJson, navyKangarooSetOverprovisioning);
  ...
  // below
  checkCorrectSize<CacheConfig, 768>(); 
