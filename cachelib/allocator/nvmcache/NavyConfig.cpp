...
ZNSKVCacheConfig& ZNSKVCacheConfig::setSizePctAndMaxItemSize(
    unsigned int sizePct, uint64_t smallItemMaxSize) {
  if (sizePct > 100) {
    throw std::invalid_argument(folly::sformat(
        "to enable ZNSKVCache, ZNSKVCache size pct should be in the range of [0, 100]"
        ", but {} is set",
        sizePct));
  }
  if (sizePct == 0) {
    XLOG(INFO) << "ZNSKVCache is not configured";
  }
  sizePct_ = sizePct;
  smallItemMaxSize_ = smallItemMaxSize;
  return *this;
...
void NavyConfig::setZNSKVCache(unsigned int ZNSKVCacheSizePct,
  ...
  ZNSKVCacheConfig_
      .setSizePctAndMaxItemSize(ZNSKVCacheSizePct, ZNSKVCacheSmallItemMaxSize)
      .setBucketSize(ZNSKVCacheBucketSize)
      .setBucketRatio(ZNSKVCacheBucketRatio)
      .setBloomFilterFp(ZNSKVCacheBloomFilterFp)
      .setElementCountPerPBF(ZNSKVCacheElementCountPerPBF)
      .setWriteBackMode(ZNSKVCacheEnableWriteBack)
      .setStartStatZone(ZNSKVCacheStartStatZone)
      .setStep(ZNSKVCacheStep)
      .setHashMode(ZNSKVCacheEnableDoubleHash)
      .setBufferMode(ZNSKVCacheEnableDoubleBuffer)
      .setKickOffMode(ZNSKVCacheEnableKickOff)
      .setMaxNumKicks(ZNSKVCacheMaxNumKicks);
}
