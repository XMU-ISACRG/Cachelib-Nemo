...
/**
 * ZNSKVCacheConfig provides APIs for users to configure Kangaroo engine, which is
 * one part of NavyConfig.
 * 
 * By this class, users can:
 * - enable ZNSKVCache by setting sizePct > 0
 * - set bucket size
 * - get the values of all the above parameters
 */
class ZNSKVCacheConfig {
  public:
    // Set ZNSKVCache device percentage and maximum item size(in bytes) to enable
    // ZNSKVCache engine. Default value of sizePct and smallItemMaxSize is 0,
    // meaning ZNSKVCache is not enabled.
    // @throw std::invalid_argument if sizePct is not in the range of
    //        [0, 100].
    ZNSKVCacheConfig& setSizePctAndMaxItemSize(unsigned int sizePct,
                                          uint64_t smallItemMaxSize);

    // Set the bucket size in bytes for ZNSKVCache engine.
    // Default value is 4096.
    ZNSKVCacheConfig& setBucketSize(uint32_t bucketSize) noexcept {
      bucketSize_ = bucketSize;
      return *this;
    }

    // Set ratio of buckets contained in PBFGList in a Zone bucketsNum range
    ZNSKVCacheConfig& setBucketRatio(uint64_t bucketRatio) noexcept {
      bucketRatio_ = bucketRatio;
      return *this;
    }

    // Set the fp of per Page BF
    ZNSKVCacheConfig& setBloomFilterFp(double BloomFilterFp) noexcept {
      BloomFilterFp_ = BloomFilterFp;
      return *this;
    }

    // Set the elementsNum of per Page BF
    ZNSKVCacheConfig& setElementCountPerPBF(uint64_t elementCountPerPBF) noexcept {
      elementCountPerPBF_ = elementCountPerPBF;
      return *this;
    }

    // Set write back mode when evicting a data zone
    ZNSKVCacheConfig& setWriteBackMode(bool enableWriteBack) noexcept {
      enableWriteBack_ = enableWriteBack;
      return *this;
    }

    // Set startStatZone
    ZNSKVCacheConfig& setStartStatZone(uint64_t startStatZone) noexcept {
      startStatZone_ = startStatZone;
      return *this;
    }

    // Set step
    ZNSKVCacheConfig& setStep(uint64_t step) noexcept {
      step_ = step;
      return *this;
    }

    ZNSKVCacheConfig& setHashMode(bool enableDoubleHash) noexcept {
      enableDoubleHash_ = enableDoubleHash;
      return *this;
    }

    ZNSKVCacheConfig& setBufferMode(bool enableDoubleBuffer) noexcept {
      enableDoubleBuffer_ = enableDoubleBuffer;
      return *this;
    }

    ZNSKVCacheConfig& setKickOffMode(bool enableKickOff) noexcept {
      enableKickOff_ = enableKickOff;
      return *this;
    }

    ZNSKVCacheConfig& setMaxNumKicks(uint64_t maxNumKicks) noexcept {
      maxNumKicks_ = maxNumKicks;
      return *this;
    }
    ...
    uint64_t getBucketRatio() const { return bucketRatio_; }

    double getBloomFilterFp() const { return BloomFilterFp_; }

    uint64_t getElementCountPerPBF() const { return elementCountPerPBF_; }

    bool getWriteBackMode() const { return enableWriteBack_; }

    uint64_t getStartStatZone() const { return startStatZone_; }

    uint64_t getStep() const { return step_; }
    
    uint64_t getSmallItemMaxSize() const { return smallItemMaxSize_; }

    bool getHashMode() const { return enableDoubleHash_; }

    bool getBufferMode() const { return enableDoubleBuffer_; }

    bool getKickOffMode() const { return enableKickOff_; }

    uint64_t getMaxNumKicks() const { return maxNumKicks_; }

  private:
    // Percentage of how much of the device out of all is given to ZNSKVCache
    // engine in Navy, e.g. 50.
    unsigned int sizePct_{0};
    // Navy ZNSKVCache engine's bucket size (must be multiple of the minimum
    // device io block size).
    // This size determines how big each bucket is.
    uint32_t bucketSize_{4096};
    // The ratio of buckets contained in PBFGList in a Zone bucketsNum range
    uint64_t bucketRatio_{1};
    // The fp of per Page BF
    double BloomFilterFp_{0.001};
    // The elementsNum of per Page BF
    uint64_t elementCountPerPBF_{40};
    // if enable writeBack mode
    bool enableWriteBack_{false};
    // evict start zone
    uint64_t startStatZone_{20};
    // evict step
    uint64_t step_{5};
    // The maximum item size to put into Navy ZNSKVCache engine.
    uint64_t smallItemMaxSize_{};

    // if enable double Hash
    bool enableDoubleHash_{false};
    // if enable double dataRegionBuffer
    bool enableDoubleBuffer_{false};
    // if kick objects when there is no enough space in dataRegion bucket
    bool enableKickOff_{false};
...
class NavyConfig {
 ...
 public:
  ...
  bool isZNSKVCacheEnabled() const { return ZNSKVCacheConfig_.getSizePct() > 0; }
  ...
  uint64_t getSmallItemThreshold() const {
    ...
    } else if (isZNSKVCacheEnabled()) {
      return ZNSKVCacheConfig_.getSmallItemMaxSize();
    } else {
      return 0;
    }
  }
  ...
  // Return a const ZNSKVCacheConfig to read values of its parameters.
  const ZNSKVCacheConfig& ZNSKVCache() const { return ZNSKVCacheConfig_; }
  ...
  // ============ ZNSKVCache settings =============
  // (Deprecated) Set the parameters for ZNSKVCache.
  // @throw std::invalid_argument if ZNSKVCacheSizePct is not in the range of
  //        0~100.
  void setZNSKVCache(unsigned int ZNSKVCacheSizePct,
                  uint64_t ZNSKVCacheSmallItemMaxSize,
                  uint32_t ZNSKVCacheBucketSize,
                  uint64_t ZNSKVCacheBucketRatio,
                  double ZNSKVCacheBloomFilterFp,
                  uint64_t ZNSKVCacheElementCountPerPBF,
                  bool ZNSKVCacheEnableWriteBack,
                  uint64_t ZNSKVCacheStartStatZone,
                  uint64_t ZNSKVCacheStep,
                  bool ZNSKVCacheEnableDoubleHash,
                  bool ZNSKVCacheEnableDoubleBuffer,
                  bool ZNSKVCacheEnableKickOff,
                  uint64_t ZNSKVCacheMaxNumKicks);
  // Return ZNSKVCacheConfig for configuration.
  ZNSKVCacheConfig& ZNSKVCache() noexcept { return ZNSKVCacheConfig_; }
 ...
 private:
  ...
  // ============ ZNSKVCache settings =============
  ZNSKVCacheConfig ZNSKVCacheConfig_{};
  ...
  // uint32_t maxConcurrentInserts_{1};
