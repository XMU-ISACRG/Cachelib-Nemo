...
uint64_t setupKangaroo(const navy::KangarooConfig& kangarooConfig,
  ...
  kangaroo->setSetOverprovisioning(kangarooConfig.getSetOverprovisioning());
  kangaroo->setThreshold(kangarooConfig.getFlushingThreshold(), kangarooConfig.getGcUpperThreshold(), kangarooConfig.getGcLowerThreshold());
  ...
  if (kangarooConfig.getLogSizePct()) {
    ...
    XLOGF(INFO, "setupKangaroo logBaseOffset zone {} kangaroo offset zone {} kangaroo total zone: {}, total zones {}", 
  ...
  return totalCacheSize - kangarooCacheSize;
}
...
void setupBlockCache(const navy::BlockCacheConfig& blockCacheConfig,
  ...
  XLOG(INFO) << "before alignDown ,blockCacheSize = " << blockCacheSize;
  ...
  if (usesZnsArg.usesZonedDevice_) {
    ...
    XLOG(INFO) << "after alignDown ,blockCacheSize = " << blockCacheSize;

    // blockCacheSize = cacheSizeAdjustment;
    XLOG(INFO) << "after cacheSizeAdjustment ,blockCacheSize = " << blockCacheSize;
...
void setupCacheProtos(const navy::NavyConfig& config,
  ...
  if (config.isKangarooEnabled()) {
    auto kangarooCacheOffset = setupKangaroo(config.kangaroo(), ioAlignSize,  //同下
