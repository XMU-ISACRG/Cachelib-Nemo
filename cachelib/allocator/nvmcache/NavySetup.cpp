...
uint64_t setupZNSKVCache(const navy::ZNSKVCacheConfig& ZNSKVCacheConfig,
                      uint32_t ioAlignSize,
                      uint64_t totalCacheSize,
                      uint64_t metadataSize,
                      cachelib::navy::CacheProto& proto,
                      ZNSArgs &usesZnsArg) {
  auto bucketSize = ZNSKVCacheConfig.getBucketSize();
  if (bucketSize != alignUp(bucketSize, ioAlignSize)) {
    throw std::invalid_argument(
        folly::sformat("Bucket size: {} is not aligned to ioAlignSize: {}",
                       bucketSize, ioAlignSize));
  ...
  // If enabled, ZNSKVCache storage starts after BlockCache's.
  const auto sizeReservedForZNSKVCache =
      totalCacheSize * ZNSKVCacheConfig.getSizePct() / 100ul;
  uint64_t ZNSKVCacheNumZones = sizeReservedForZNSKVCache / usesZnsArg.dev_->getIOZoneCapSize();
  // because size calculated by sizePct may not be multiples of zone capacity
  uint64_t ZNSKVCacheCacheSize = ZNSKVCacheNumZones * usesZnsArg.dev_->getIOZoneCapSize();
  uint64_t ZNSKVCacheZoneOffset = totalCacheSize / usesZnsArg.dev_->getIOZoneCapSize() - ZNSKVCacheNumZones;
  uint64_t ZNSKVCacheCacheOffset = ZNSKVCacheZoneOffset * usesZnsArg.dev_->getIOZoneSize();

  auto ZNSKVCache = cachelib::navy::createZNSKVCacheProto();
  ZNSKVCache->setLayout(ZNSKVCacheCacheOffset, ZNSKVCacheCacheSize, bucketSize, ZNSKVCacheZoneOffset);
  
  ZNSKVCache->setBloomFilter(ZNSKVCacheConfig.getBloomFilterFp(), 
                             ZNSKVCacheConfig.getElementCountPerPBF());

  ZNSKVCache->setBucketRatio(ZNSKVCacheConfig.getBucketRatio());

  ZNSKVCache->setWritebackMode(ZNSKVCacheConfig.getWriteBackMode());

  ZNSKVCache->setStartStatZone(ZNSKVCacheConfig.getStartStatZone());

  ZNSKVCache->setStep(ZNSKVCacheConfig.getStep());

  ZNSKVCache->setHashMode(ZNSKVCacheConfig.getHashMode());

  ZNSKVCache->setBufferMode(ZNSKVCacheConfig.getBufferMode());

  ZNSKVCache->setKickOffMode(ZNSKVCacheConfig.getKickOffMode());

  ZNSKVCache->setMaxNumKicks(ZNSKVCacheConfig.getMaxNumKicks());

  proto.setZNSKVCache(std::move(ZNSKVCache), ZNSKVCacheConfig.getSmallItemMaxSize());

  if (ZNSKVCacheCacheOffset <= metadataSize) {
    throw std::invalid_argument("NVM cache size is not big enough!");
  }
  XLOG(INFO) << "metadataSize: " << metadataSize
             << " ZNSKVCacheCacheOffset: " << ZNSKVCacheCacheOffset
             << " ZNSKVCacheCacheSize: " << ZNSKVCacheCacheSize
             << " ZNSKVCacheCache contains " << (ZNSKVCacheCacheSize / 1129316352) << " zones";
  return ZNSKVCacheCacheOffset;
}
...
void setupCacheProtos(const navy::NavyConfig& config,
  ...
  // choose one from BigHash / Kangaroo / ZNSKVCache
  ...
  if (config.isBigHashEnabled()) {
    ...
    blockCacheSize = bigHashCacheOffset - metadataSize;
      ...
      // Set up ZNSKVCache if enabled
      if (config.isZNSKVCacheEnabled()) {
        auto ZNSKVCacheOffset = setupZNSKVCache(config.ZNSKVCache(), ioAlignSize,
                                              totalCacheSize, metadataSize, 
                                              proto, useZnsArgs);
        blockCacheSize = (ZNSKVCacheOffset / useZnsArgs.dev_->getIOZoneSize()) * useZnsArgs.dev_->getIOZoneCapSize() - metadataSize;
      } else {
        XLOG(INFO) << "metadataSize: " << metadataSize << ". No ZNSKVCache.";
        blockCacheSize = totalCacheSize - metadataSize;
      }
  }
