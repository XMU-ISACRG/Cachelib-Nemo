...
Blob NvmCache<C>::makeBlob(const Item& it) {
  // XLOG(INFO) << "[makeBlob]: it.getSize()   " << it.getSize();   // 100
  return Blob{
      // User requested size
      it.getSize(),
...
std::unique_ptr<NvmItem> NvmCache<C>::makeNvmItem(const WriteHandle& hdl) {
  ...
  if (item.hasChainedItem()) {
    ...
    // XLOG(INFO) << "[NvmCache<C>::makeNvmItem()]    blob.origAllocSize: " << blob.origAllocSize; //100
    const size_t bufSize = NvmItem::estimateVariableSize(blob);
    // XLOG(INFO) << "[NvmItem::estimateVariableSize]     sizeof(BlobInfo): " << sizeof(BlobInfo);  //8
    // XLOG(INFO) << "[NvmItem::estimateVariableSize]     blob.data.size(): " << blob.data.size();  //169
    // XLOG(INFO) << "[NvmCache<C>::makeNvmItem()]     bufSize: " << bufSize; //177
    return std::unique_ptr<NvmItem>(new (bufSize) NvmItem(
...
void NvmCache<C>::put(WriteHandle& hdl, PutToken token) {
  ...
  // XLOG(INFO) << "[NvmCache<C>::put()] valSize: " << valSize;
...
void NvmCache<C>::onGetComplete(GetCtx& ctx,
  ...
  if (status != navy::Status::Ok) {
    ...
    if (status != navy::Status::NotFound) {
      XLOG(INFO) << "createDeleteTombStone1";
  ...
  if (!it) {
    ...
    XLOG(INFO) << "createDeleteTombStone2";
    remove(hk, createDeleteTombStone(hk));
    return;
  }
