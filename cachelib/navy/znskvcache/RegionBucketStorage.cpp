...
namespace cachelib {
namespace navy {
static_assert(sizeof(RegionBucketStorage) == 12,
              "RegionBucketStorage overhead. Changing this may require changing "
              "the sizes used in unit tests as well");

const uint32_t RegionBucketStorage::kAllocationOverhead = sizeof(RegionBucketStorage::Slot);  
...
RegionBucketStorage::Allocation RegionBucketStorage::allocate(uint32_t size) {
  if (!canAllocate(size)) {
    return {};
  }
...
void RegionBucketStorage::remove(Allocation alloc) {
...
void RegionBucketStorage::removeUntil(Allocation alloc) {
...
RegionBucketStorage::Allocation RegionBucketStorage::getFirst() const {
  if (endOffset_ == 0) {
    return {};
  }
...
RegionBucketStorage::Allocation RegionBucketStorage::getNext(
    RegionBucketStorage::Allocation alloc) const {
  if (alloc.done()) {
    return {};
  }
