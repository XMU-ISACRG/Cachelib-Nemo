...
// ZNSKVCache engine proto. ZNSKVCache is used to cache small objects (under 2KB)
// more efficiently than BigHash by itself.
// User sets up this proto object and passes it to CacheProto::setZNSKVCache.
class ZNSKVCacheProto {
 public:
  virtual ~ZNSKVCacheProto() = default;

  // Set cache layout. Cache will start at @baseOffset and will be @size bytes
  // on the device. ZNSKVCache divides its device spcae into a number of fixed size
  // buckets, represented by @bucketSize. All IO happens on bucket-size
  // granularity.
  virtual void setLayout(uint64_t baseOffset,
                         uint64_t size,
                         uint32_t bucketSize,
                         uint64_t dataZoneOffsetId) = 0;
  ...
  // Enable Bloom filter with @fp and @elementCountPerPBF
  virtual void setBloomFilter(double fp,
                              uint64_t elementCountPerPBF) = 0;

  virtual void setBucketRatio(uint64_t bucketRatio) = 0;

  virtual void setWritebackMode(bool enableWriteback) = 0;

  virtual void setStartStatZone(uint64_t startStatZone) = 0;

  virtual void setStep(uint64_t startStep) = 0;
  
  virtual void setHashMode(bool enableDoubleHash) = 0;

  virtual void setBufferMode(bool enableDoubleBuffer) = 0;

  virtual void setKickOffMode(bool enableKickOff) = 0;

  virtual void setMaxNumKicks(uint64_t maxNumKicks) = 0;
};
...
class CacheProto {
 public:
  ...
  // Set up ZNSKVCache engine.
  virtual void setZNSKVCache(std::unique_ptr<ZNSKVCacheProto> proto,
                          uint32_t smallItemMaxSize) = 0;
...
// Creates ZNSKVCache engine prototype
