...
class BlockCacheProtoImpl final : public BlockCacheProto {
 public:
  ...
  void setLayout(uint64_t baseOffset,
    ...
    XLOG(INFO) << "r[Block cache]: " << "cacheBaseOffset: " << config_.cacheBaseOffset << ", cacheSize: " << config_.cacheSize;
    XLOG(INFO) << "r[Block cache]: " << "cacheBaseOffset(zones): " << (config_.cacheBaseOffset / 2147483648) << ", cacheSize(zone): " << (config_.cacheSize / 1129316352);
    XLOG(INFO) << "r[Block cache]: " << config_.getNumRegions();
  }
...
class ZNSKVCacheProtoImpl final : public ZNSKVCacheProto {
 public:
  ZNSKVCacheProtoImpl() = default;
  ~ZNSKVCacheProtoImpl() override = default;

  void setLayout(uint64_t baseOffset,
                 uint64_t size,
                 uint32_t bucketSize,
                 uint64_t dataZoneOffsetId) override {
    config_.cacheBaseOffset = baseOffset;
    config_.cacheSize= size;
    config_.bucketSize = bucketSize;
    config_.dataZoneOffsetId = dataZoneOffsetId;
  }
  ...
  void setBloomFilter(double fp, uint64_t elementCountPerPBF) override {
    // Want to make @setLayout and Bloom filter setup independent.
    bloomFilterEnabled_ = true;
    config_.fpProb = fp;
    config_.elementCount = elementCountPerPBF;
  }

  void setDevice(Device* device) { config_.device = device; }

  void setBucketRatio(uint64_t bucketRatio) { config_.bucketRatio = bucketRatio; }

  void setWritebackMode(bool enableWriteback) { config_.writeBackMode = enableWriteback; }

  void setStartStatZone(uint64_t startStatZone) { config_.startStatZone = startStatZone; }
  
  void setStep(uint64_t step) { config_.step = step; }

  // void setDestructorCb(DestructorCallback cb) {
  //   config_.destructorCb = std::move(cb);
  // }

  void setHashMode(bool enableDoubleHash) { config_.isEnableDoubleHash = enableDoubleHash;}

  void setBufferMode(bool enableDoubleBuffer) { config_.isEnableDoubleBuffer = enableDoubleBuffer;}

  void setKickOffMode(bool enableKickOff) { config_.isEnableKickOff = enableKickOff;}

  void setMaxNumKicks(uint64_t maxNumKicks) { config_.maxNumKicks = maxNumKicks;}
  ...
  std::unique_ptr<Engine> create() && {
    ...
    return std::make_unique<ZNSKVCache>(std::move(config_));
  }

 private:
  ZNSKVCache::Config config_; //
  bool bloomFilterEnabled_{false};
...
class CacheProtoImpl final : public CacheProto {
 public:
  ...
  void setZNSKVCache(std::unique_ptr<ZNSKVCacheProto> proto,
          uint32_t smallItemMaxSize) override {
      ZNSKVCacheProto_ = std::move(proto);
      config_.smallItemMaxSize = smallItemMaxSize;
  }
  ...
  std::unique_ptr<AbstractCache> create() && {
    ...
    if (ZNSKVCacheProto_) {
      auto ZNSKVCacheProto = dynamic_cast<ZNSKVCacheProtoImpl*>(ZNSKVCacheProto_.get());
      if (ZNSKVCacheProto != nullptr) {
        ZNSKVCacheProto->setDevice(config_.device.get());
        config_.smallItemCache = std::move(*ZNSKVCacheProto).create();
      }
    }
 ...
 private:
  ...
  std::unique_ptr<ZNSKVCacheProto> ZNSKVCacheProto_;
...
std::unique_ptr<ZNSKVCacheProto> createZNSKVCacheProto() {
  return std::make_unique<ZNSKVCacheProtoImpl>();
}
