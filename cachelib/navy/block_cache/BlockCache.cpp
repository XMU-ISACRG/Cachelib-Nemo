...
BlockCache::BlockCache(Config&& config, ValidConfigTag)
  ...
  XLOG(INFO) << "[Block cache](constructor): " << "cacheBaseOffset: " << config.cacheBaseOffset << ", cacheSize: " << config.cacheSize;
  XLOG(INFO) << "[Block cache](constructor): " << "cacheBaseOffset(zones): " << (config.cacheBaseOffset / 2147483648) << ", cacheSize(zones): " << (config.cacheSize / 1129316352);
  XLOG(INFO) << "[Block cache](constructor): " << config.getNumRegions();
...
Status BlockCache::insert(HashedKey hk, BufferView value) {
  // XLOG(INFO) << "============BlockCache::insert================";
  // if (insertCount_.get() % 20000 == 10) {
  //     XLOG(INFO) << "============BlockCache::insert================";
  //     XLOGF(INFO, "insert key {} at insertCount:{}", hk.keyHash(), insertCount_.get());
  // }
...
bool BlockCache::couldExist(HashedKey hk) {
  ...
  // if (insertCount_.get() % 20000 == 10) {
  //   XLOG(INFO) << "============BlockCache::couldExist================";
  //   XLOGF(INFO, "couldExist key {} at lookupCount_:{}", hk.keyHash(), lookupCount_.get());
  // }
  return true;
}
...
Status BlockCache::lookup(HashedKey hk, Buffer& value) {
  ...
  // if (lookupCount_.get() % 20000 == 10) {
  //   XLOG(INFO) << "============BlockCache::lookup================";
  //   XLOGF(INFO, "lookup key {} at lookupCount_:{}", hk.keyHash(), lookupCount_.get());
  // }
