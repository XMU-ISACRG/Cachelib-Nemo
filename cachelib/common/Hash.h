...
public:
    static HashedKey precomputed(folly::StringPiece key, uint64_t keyHash) {
        return HashedKey{std::string(key.data(), key.size()), keyHash};
    }

    explicit HashedKey(folly::StringPiece key)
        : HashedKey{std::string(key.data(), key.size()), hashBuffer(key)} {}

    HashedKey(const char* key, size_t size)
        : HashedKey{std::string(key, size), hashBuffer(folly::StringPiece(key, size))} {}

    std::string key() const { return key_data_; }

    folly::StringPiece keyView() const { return folly::StringPiece(key_data_); }

    uint64_t keyHash() const { return keyHash_; }

    uint64_t keyHash2() const { return hashBuffer2(folly::StringPiece(key_data_)); }

    bool operator==(HashedKey other) const {
        return keyHash_ == other.keyHash_ && 
...
private:
    HashedKey(std::string key_data, uint64_t keyHash)
    ...
    static uint64_t hashBuffer2(folly::StringPiece key, uint64_t seed = 0) {
        return folly::hash::SpookyHashV1::Hash64(key.data(), key.size(), seed);
    }
