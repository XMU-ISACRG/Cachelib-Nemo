...
class BloomFilter {
 public:
  ...
  void getAllHashIdx(uint64_t key, std::vector<uint64_t>& hashAddress_);
  bool existObj(uint32_t idx, uint64_t key, std::vector<uint64_t>& hashAddress_);
  //get the bit array
  uint8_t* getBitArray() const {
    XDCHECK(bits_);
    return bits_.get();
  }

  //set the bit array
  void setBitArray(navy::Buffer& buffer) const {
    std::memcpy(bits_.get(), buffer.data(), getByteSize());
  }
