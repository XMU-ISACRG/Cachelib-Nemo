...
BloomFilter::BloomFilter(uint32_t numFilters,
  ...
  XLOG(INFO) << "numFilters: " << numFilters << ", numHashes * hashTableBitSize: " << numHashes * hashTableBitSize;
...
BloomFilter::BloomFilter(const BloomFilter& other)
    : numFilters_(other.numFilters_),
      ...
      seeds_(other.seeds_),  
      bits_(other.bits_ ? std::make_unique<uint8_t[]>(other.getByteSize()) : nullptr) {
  if (other.bits_) {
    std::memcpy(bits_.get(), other.bits_.get(), other.getByteSize());
  }
}
...
void BloomFilter::getAllHashIdx(uint64_t key, std::vector<uint64_t>& hashAddress_) {
  for (auto seed : seeds_) {
    auto bitNum = facebook::cachelib::combineHashes(key, seed) % hashTableBitSize_;
    hashAddress_.push_back(bitNum);
  }  
}


bool BloomFilter::existObj(uint32_t idx, uint64_t key, std::vector<uint64_t>& hashAddress_) {
  size_t firstBit = 0;
  const auto* filterPtr = getFilterBytes(idx);
  for (auto i : hashAddress_) {
    if (!bitGet(filterPtr, firstBit + i)) {
      return false;
    }
    firstBit += hashTableBitSize_;
  }
